...
@objc(BlockingBridge)
class BlockingBridge: NSObject, RCTBridgeModule {
  static func moduleName() -> String! { return "BlockingBridge" }
  ...
  private static func reconfigureShields(with selection: FamilyActivitySelection) -> Bool {
    ...
    if applied {
      Self.logger.info("Shields applied — ShieldConfig extension provides custom UI")
    }
